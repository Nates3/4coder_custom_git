    ...
    switch (in.event.kind)
    {
      ...
      case InputEventKind_KeyStroke:
      {
        ...
        switch (in.event.key.code)
        {
          ...
          {
            /**
             * Adds J,K movement inside lists, needs to be at the bottom
             * because default should be overwritten when they have
             * KeyCode_Control modifiers
         */
            if (in.event.key.code == LISTER_MOVE_UP_KEY &&
                has_modifier(&in.event.key.modifiers, KeyCode_Control))
            {
              if (lister->handlers.navigate != 0)
              {
            ...
            if (in.event.key.code == LISTER_MOVE_DOWN_KEY &&
                has_modifier(&in.event.key.modifiers, KeyCode_Control))
            {
              if (lister->handlers.navigate != 0)
              {
