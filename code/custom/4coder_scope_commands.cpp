...
struct Surrounding_Range
{
  b32 found;
  Range_i64 r;
};

function Surrounding_Range 
get_surrounding_range(Application_Links *app, Buffer_ID buffer, i64 pos)
{
  Surrounding_Range range = {};
  if (find_surrounding_nest(app, buffer, pos, FindNest_Scope, &range.r))
  {
    range.found = true;
    for (;;){
      pos = range.r.min;
