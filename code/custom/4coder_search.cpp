...
find_all_matches_all_editing_buffers(Application_Links *app, Arena *arena, String_Const_u8_Array match_patterns, String_Match_Flag must_have_flags, String_Match_Flag must_not_have_flags){
 ...
 for (Buffer_ID buffer = get_buffer_next(app, 0, Access_Always);
  ...
  if(buffer != jump_buffer &&
     !buffer_has_name_with_star(app, buffer)) {
...
print_all_matches_all_buffers(Application_Links *app, String_Const_u8_Array match_patterns, String_Match_Flag must_have_flags, String_Match_Flag must_not_have_flags, Buffer_ID out_buffer_id){
 Scratch_Block scratch(app);
 String_Match_List matches = find_all_matches_all_editing_buffers(app, scratch, match_patterns, must_have_flags, must_not_have_flags);
   ...
   switch (in.event.kind){
    ...
    case InputEventKind_KeyStroke:
    {
     switch (in.event.key.code){
      case KeyCode_Return:
      {
       result.text = menu.options[menu_f1_index];
       result.range = range;
       keep_looping_menu = false;
      }break;
      ...
      case KeyCode_F8:
      {
       menu_f1_index = (in.event.key.code - KeyCode_F1);
       result.text = menu.options[menu_f1_index];
       result.range = range;
       keep_looping_menu = false;
      }break;
