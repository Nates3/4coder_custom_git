...
parse_buffer_to_jump_array_not_slow(Application_Links *app, Arena *arena, Buffer_ID buffer)
{
	...
	i64 min_line = clamp_bot(view_line_number - JUMP_BUFFER_RANGE, 1);
	i64 max_line = clamp_top(view_line_number + JUMP_BUFFER_RANGE, max_buffer_line);
	
	for(i32 line = (i32)min_line;
			line < (i32)max_line;
			++line)
	{
		b32 output_jump = false;
...
internal void
init_marker_list_not_slow(Application_Links *app, Heap *heap, Buffer_ID buffer, Marker_List *list)
{
	Scratch_Block scratch(app);
	
	Sticky_Jump_Array jumps = parse_buffer_to_jump_array_not_slow(app, scratch, buffer);
...
internal Marker_List*
always_make_new_list_for_buffer(Application_Links *app, Heap *heap, Buffer_ID buffer_id){
	Marker_List *result = get_marker_list_for_buffer(buffer_id);
	if (result != 0)
	{
		delete_marker_list(result);
	...
	if (result == 0){
		result = make_new_marker_list_for_buffer(heap, buffer_id);
		init_marker_list_not_slow(app, heap, buffer_id, result);
		if (result->jump_count == 0){
			delete_marker_list(result);
			result = 0;
		}
...
internal Marker_List *
make_list_for_buffer(Application_Links *app, Heap *heap, Buffer_ID buffer_id)
{
	Marker_List *result = make_new_marker_list_for_buffer(heap, buffer_id);
	init_marker_list_not_slow(app, heap, buffer_id, result);
	if (result->jump_count == 0){
		delete_marker_list(result);
		result = 0;
...
get_locked_jump_state(Application_Links *app, Heap *heap){
	...
	if (result.view != 0){
		...
		result.list = always_make_new_list_for_buffer(app, heap, buffer);
