	...
	for (;;)
	{
		...
		else
		{
				 ...
				 !HasFlag(event_properties, EventProperty_MouseMove))
			{
				...
				if(multi_cursor_mode == Multi_Cursor_PlaceCursors || multi_cursor_mode == Multi_Cursor_Enabled)
				{
...
default_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
                      ...
                      Rect_f32 rect)
{
	ProfileScope(app, "default render buffer");
...
set_all_default_hooks(Application_Links *app)
{
	...
	set_custom_hook(app, HookID_RenderCaller, q7_render_caller);
