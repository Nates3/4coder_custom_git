...
struct Token_Iterator_Array{
  u64 user_id;
  // NOTE(nates): "tokens" token ptr might be the start of token array
  // "ptr" token ptr might be our index into that array just as a ptr
  // "token_it_inc_all" function advances the pointer
  Token *ptr;
  Token *tokens;
  i64 count;
};
