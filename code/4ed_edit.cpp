...
edit_fix_markers(Thread_Context *tctx, Models *models, Editing_File *file, Batch_Edit *batch)
{
			 ...
			 panel = layout_get_next_open_panel(layout, panel))
	{
		...
		cursor_max += view->cursor_count*2; // NOTE(nates): Times 2 to include marks
	}
	
	//i32 cursor_max = layout_get_open_panel_count(layout)*4; // TODO(nates): what does this 4 mean?
	...
	for (Panel *panel = layout_get_first_open_panel(layout);
		...
		if (view->file == file)
		{
			...
			for(u32 multi_cursor_index = 0;
					...
					++multi_cursor_index)
			{
				write_cursor_with_index(cursors, &cursor_count, (i32)edit_pos.cursors[multi_cursor_index]);
				write_cursor_with_index(cursors, &cursor_count, (i32)view->marks[multi_cursor_index]);
			}
		...
		for (Panel *panel = layout_get_first_open_panel(layout);
			...
			if (view->file == file)
			{
						...
						++multi_cursor_index)
				{
					...
					i64 mark_pos = cursors[cursor_count++].pos;
					
					Multi_Cursor_Mode multi_cursor_mode = view->multi_cursor_mode;
					if(multi_cursor_mode == Multi_Cursor_Disabled)
					{
						view_set_cursor_and_scroll(tctx, models, view, cursor_pos, edit_pos.scroll);
						view->marks[0] = mark_pos;
					}
					else if(multi_cursor_mode == Multi_Cursor_Enabled)
					{
						view->edit_pos_.cursors[multi_cursor_index] = cursor_pos;
						view->marks[multi_cursor_index] = mark_pos;
					}
				}
