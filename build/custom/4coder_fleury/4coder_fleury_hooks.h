...
function DELTA_RULE_SIG(F4_DeltaRule)
{
    ...
    Smooth_Step step_x = smooth_camera_step(pending.x, velocity->x, 80.f, 1.f); // NOTE(nates) Smooth scrolling is this timestep value
    Smooth_Step step_y = smooth_camera_step(pending.y, velocity->y, 80.f, 1.f);
